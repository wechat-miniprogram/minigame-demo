...
let getSelfPromise: Promise<any> | undefined;
 ...
 */
function getWxGameData(item: any) {
  let source;
  try {
 ...
 */
// function rankDataFilter(res: any, selfUserInfo: any = false) {
//   const data = (res.data || []).filter((item: any) => item.KVDataList && item.KVDataList.length);
 ...
 */
// export function getFriendRankData(key: any, needMarkSelf = true) {
 ...
 */
// export function getGroupFriendsRankData(shareTicket: string, key: string, needMarkSelf = true) {
 ...
 */
// export function setUserRecord(key: string, score = 0, extra = {}) {
